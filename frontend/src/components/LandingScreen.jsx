...
import { ArrowUpRight, Globe2, HeartHandshake, LockKeyhole, Sparkles, Users, Phone, User } from 'lucide-react';
...
export default function LandingScreen({ onStart, onAbout }) {
  ...
  const handleAuth = async (e) => {
    ...
    const body = authMode === 'signup' ? { username, emergencyContact } : { username };
    
    try {
      // In dev, assuming proxy or same origin port 3000
      ...
      localStorage.setItem('safeSpeak_user_profile', JSON.stringify({
        userId: data.user.userId,
        alias: generateAlias(), // Generate a cool random name for the session
      ...
      <div className="w-full max-w-5xl">
        ...
        <div className="grid items-center gap-12 text-left md:grid-cols-[1.25fr_.75fr] md:gap-20">
          <div>
            ...
            <div className="mt-9 reveal reveal-delay-3">
              {!authMode ? (
                <div className="flex flex-wrap items-center gap-4">
                  <button onClick={() => setAuthMode('login')} className="group inline-flex items-center gap-4 rounded-full bg-[#e8795d] px-6 py-4 font-semibold text-[#101a1a] shadow-[0_12px_30px_rgba(232,121,93,.2)] transition hover:-translate-y-1 hover:bg-[#f28e73]">Login <span className="grid h-8 w-8 place-items-center rounded-full bg-[#101a1a] text-[#f6f2e9] transition group-hover:rotate-45"><ArrowUpRight size={16} /></span></button>
                  <button onClick={() => setAuthMode('signup')} className="inline-flex items-center gap-4 rounded-full bg-[#91bb8e] px-6 py-4 font-semibold text-[#101a1a] shadow-[0_12px_30px_rgba(145,187,142,.2)] transition hover:-translate-y-1 hover:bg-[#a3c4ac]">Sign Up <span className="grid h-8 w-8 place-items-center rounded-full bg-[#101a1a] text-[#f6f2e9] transition group-hover:rotate-45"><ArrowUpRight size={16} /></span></button>
                ...
                <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
                  <div className="mb-6 flex gap-4 border-b border-white/10 pb-4">
                    <button onClick={() => { setAuthMode('login'); setError(''); }} className={`font-semibold ${authMode === 'login' ? 'text-[#e8795d]' : 'text-white/50 hover:text-white'}`}>Login</button>
                    <button onClick={() => { setAuthMode('signup'); setError(''); }} className={`font-semibold ${authMode === 'signup' ? 'text-[#e8795d]' : 'text-white/50 hover:text-white'}`}>Sign Up</button>
                  </div>
                  ...
                  <form onSubmit={handleAuth} className="flex flex-col gap-4">
                    ...
                    <div>
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 flex items-center pl-4 text-white/50"><User size={16} /></div>
                        <input type="text" required placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/5 pl-11 pr-4 py-3 text-sm text-[#f6f2e9] outline-none transition focus:border-[#e8795d]" />
                      </div>
                    </div>
                    ...
                    {authMode === 'signup' && (
                      <div>
                        <div className="relative">
                          <div className="absolute inset-y-0 left-0 flex items-center pl-4 text-white/50"><Phone size={16} /></div>
                          <input type="tel" required placeholder="Emergency Contact" value={emergencyContact} onChange={(e) => setEmergencyContact(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/5 pl-11 pr-4 py-3 text-sm text-[#f6f2e9] outline-none transition focus:border-[#e8795d]" />
                        </div>
                      </div>
                    )}
                    
                    <button type="submit" className="mt-2 w-full rounded-2xl bg-[#e8795d] py-3 font-semibold text-[#101a1a] transition hover:bg-[#f28e73]">{authMode === 'login' ? 'Login' : 'Create Account'}</button>
                    <button type="button" onClick={() => setAuthMode(null)} className="w-full text-xs text-white/50 hover:text-white">Cancel</button>
                  </form>
                </div>
              )}
            </div>
